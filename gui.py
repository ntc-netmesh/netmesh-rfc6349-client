...
def rev(lat, lon):
    print("reverse mode")
    asyncio.get_event_loop().run_until_complete(r_to_l(lat, lon))
    ...
    async with websockets.connect(ws_url, ping_timeout=600) as websocket:
        ...
        try:
            ...
            if "start iperf tcp" in state:
                ...
                p = subprocess.Popen(["iperf3", "-c", server_ip, "-t", "10", "-b", "1000M", "-w", str(rwnd)+"K", "-R", "-f", "m"], stdout = subprocess.PIPE)
                p.wait()
            ...
            while True:
                ...
                if "start iperf tcp" in state:
                    ...
                    #TCP mode - run for 5s to keep pcap small
                    eel.printprogress("iPerf TCP started")
                    p = subprocess.Popen(["iperf3", "-c", server_ip, "-t", "10", "-b", "1000M", "-w", rwnd+"K", "-R", "-f", "m"], stdout = subprocess.PIPE)
                    p.wait()
                    await websocket.send("iperf tcp done")
        ...
        try:
            ...
            if state == "iperf server started":
                ...
                while True:
                    ...
                    p = subprocess.Popen(["iperf3", "-c", server_ip, "-t", "10", "-w", rwnd+"K", "-f", "m", "-b", "1000M"], stdout = subprocess.PIPE)
                    speed_plot = []
                    p.wait()
        ...
        try:
            ...
            while True:
                ...
                if state == "iperf udp started":
                    command = [
                        'iperf3 -c ' + server_ip + ' -p 4001 -t 10 -u -b 1000M -f m',
                        'iperf3 -c ' + server_ip + ' -p 4002 -t 10 -u -b 1000M -R -f m'
                    ]
            ...
            if "rwnd" in state:
                ...
                command = [
                    ...
                    'iperf3 -c ' + server_ip + ' -p 4001 -t 10 -w ' + str(normal_wnd_size) + 'K -f m',
                    'iperf3 -c ' + server_ip + ' -p 4002 -t 10 -w ' + reverse_wnd_size + 'K -R -f m', 
                ]
...
eel.start('hello.html', size=(1100,600))             # Start (this blocks and enters loop)
