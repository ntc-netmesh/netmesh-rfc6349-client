...
class NetMeshConfig:
    __CONFIG_FILE_DIRECTORY = f'/home/{netmesh_utils.get_user()}/.config/netmesh-rfc6349-app'
    ...
    class _NetMeshDeviceConfig:
        ...
        def get_device_name(self):
            device_name = ""
            try:
                device_name = self._config.get(self._SECTION_NAME, "device_name")
    ...
    class _NetMeshUsersConfig:
            ...
            try:
                ...
                for i, user in enumerate(logged_users):
                    if "token_expiry" in user:
                        ...
                        user.update({'token_expiry_seconds': token_expiry_seconds})
                        logged_users[i] = user
            except Exception as ex:
                print(ex)
        ...
        def delete_logged_user(self, user_email: str):
            ...
            self._config.set("USERS", "logged_users", json.dumps(remaining_logged_users))
...
if __name__ == "__main__":
    ...
    users = users_config.get_logged_users()
    ...
    # device_config.set_device_name("albert-laptop")
    
    # users_config = config.load_users_config()
    # users_config.add_logged_user({
    #     "name": "Albert Francisco",
    #     "email": "albert.francisco@gmail.com",
    # })
