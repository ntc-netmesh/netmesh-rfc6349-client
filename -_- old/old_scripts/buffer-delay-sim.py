...
while (count < packettotal):
	pkt1 = packets[count]
	try:
		##scan for client packets
		if IP in pkt1:
			if ((TCP in pkt1)) and (pkt1[IP].src == src_ip_addr) and (pkt1[TCP].dport == port):
			...
			##scan for server acks
			if ((TCP in pkt1)) and (pkt1[IP].src == dest_ip_addr and pkt1[TCP].flags == 'A') and (pkt1[TCP].sport == port):
...
print("ave RTT: " + str((sum(rtt)/len(rtt))*1000) + " ms")
